...
Both builds must retain the same damage element and base Character/weapon scaling stat. The comparison boundary validates both equipped loadouts with the existing canonical `validateEchoLoadout`; caller-qualified equipment cannot override its COST limit. This corrects the build-lead finding on reviewed head `afcd97f5fda9b1b4aecaa039cb621174ca747faf`, which accepted individually exact five-card COST-13 loadouts. Stat-card reconstruction remains independent of equipped legality, and one-slot/same-COST replacement remains required. Unsupported facts, S1/S2 inputs, incomplete context, stale keys, changed non-selected cards or ambiguous hit counts fail closed. A PENDING scenario returns reasons without either damage value or a delta. Healing/ER stats remain visible in the projection but cannot establish resource feasibility. Results explicitly deny rotation-DPS and whole-build upgrade authorization; zero current damage has no invented percentage denominator.

Local validation after the COST review fix: 864/864 tests, all nine repository audits, strict build and whitespace pass. Regression cases cover all 492 supported facts, ATK/HP/DEF, automatic secondary stats, source damage class, nonlinear crit, independently changing effects, pending/stale context, rejected over-COST current/candidate loadouts and the legal five-card COST-12 path. Existing Ciaccona, owned-build comparisons and readiness retain their expected results. Exact final-head Verify/Export, Chrome regressions, artifact parity and fully paginated review status are recorded on PR197 and in the Handoff.
