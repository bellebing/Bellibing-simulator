...
`UNREVIEWED + SEMANTICALLY_REVIEWED_IMPLEMENTATION_PENDING` gives **26 actionable shared edges**. This is a prioritization signal only; closure work optimizes for shortest verified route to `DPS_READY` plus dependency reuse. The semantic partition does not remove canonical pending IDs. The historical #190 baseline had 30 unreviewed / 20 primitive-available; merged #191 added exact Echo attack facts for Cantarella, Zhezhi and Carlotta without closing their timelines or changing any pending ID.
