...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `1b32e30ddf39dbe964b545934a38d1d83df67bea`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1189](https://github.com/bellebing/Bellibing-simulator/actions/runs/35310604719) and [Export #1088](https://github.com/bellebing/Bellibing-simulator/actions/runs/35310604666) passed on that exact runtime head with **912/912 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

The candidate source-qualifies partial non-Echo context for all 54 existing S0/max-skill Character hit consumers / 492 facts: level-90 Character/core weapon stats, max Minor Fortes, 60 permanent weapon effects, 30 static Sonata effects, 50 reviewed static main-Echo effects, weapon cast/damage/heal windows, Sonata cast/Rejuvenating Glow healing, bounded Sonata damage, explicit target-qualified Sonata windows, and the two reviewed cross-owner Sonata Outro transfer stats. Current and candidate builds retain independent equipment/event proof; missing event/state/source scope remains named PENDING rather than zero.

Cross-owner Sonata transfer composition requires a canonical released source owner, explicit source 5-piece equipment evidence, actual Outro -> selected incoming Character identity, isolated activation/no-later-activation proof and hit-time ordering independently per build. Duplicate same-effect source activations are rejected because stacking is unreviewed. The incoming Character's preset does not prove teammate equipment, so exact Export artifact `10533077820` intentionally reports no derived cross-owner preset reach and independent reconstruction confirms **PARTIAL_L3 = 54** and **PARTIAL_L4 = 21**.

This remains partial isolated-hit context only: complete L3–L7 stays two (Augusta/Ciaccona), readiness stays 43/3/9/2, all 83 edges / 72 dependency IDs and the six Reference Team blockers remain open. Character Outro amplification remains outside the generic hit context until scoped stacking/overlap semantics are proven. The next bounded candidate lane is the three reviewed ordinary-stat Echo incoming transfers with explicit source main-Echo/rank/arm/recipient evidence; no preset reach may be inferred from team membership. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
