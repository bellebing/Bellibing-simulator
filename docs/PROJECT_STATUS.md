...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `22d2a748978cccec64345187671ba0785af0c7b6`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1261](https://github.com/bellebing/Bellibing-simulator/actions/runs/35331016298) and [Export #1160](https://github.com/bellebing/Bellibing-simulator/actions/runs/35331016116) passed on that exact runtime head with **975/975 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

The candidate source-qualifies partial non-Echo context for all 54 existing S0/max-skill Character hit consumers / 492 facts. The typed scoped-amplification boundary still leaves the one-scalar damage kernel unchanged: zero or one active applicable reviewed term may populate `amplification`; multiple applicable active terms are `PENDING_STACKING`, never guessed. An active source-qualified term requires residual caller amplification = 0.

The amplification layer now includes Shorekeeper Binary Butterfly, Bloodpact's Pledge `BPP-TEAM-AERO`, **12 generic Character Outro facts / 19 Character-hit-compatible terms**, and Iuno's separate Heavy Outro transfer. Generic Character Outros require actual incoming recipient, explicit recipient switch-out history and source-specific isolated-activation proof where reviewed. Qiuyuan remains excluded because its reviewed Outro term is Echo Skill-only. Dual-term source Outros remain separate: exactly one applicable term can resolve for the selected hit; two applicable terms stay pending.

Iuno uses its dedicated reviewed transfer/lifecycle adapter rather than the generic parser path. It requires exact Iuno -> incoming recipient identity, explicit switch-out history, conservative `NONE_ACTIVE` and no-later-activation proof plus same-timestamp ordering. This generic capability does **not** close `iuno-wan-light-augusta-event-overlap`, Jiyan's Iuno timing gap, or any other profile overlap dependency.

Exact Export artifact `10541161001` contains one `iunoOutroAmplification` support row and **19 `characterOutroAmplification` rows over 12 facts**. Cross-owner/event capabilities remain excluded from preset-derived reach, so **PARTIAL_L3 = 54** and **PARTIAL_L4 = 21**. Complete L3–L7 stays two (Augusta/Ciaccona), readiness stays 43/3/9/2, all 83 edges / 72 dependency IDs and all six Reference Team blockers remain open.

Fresh reranking identifies Lux & Umbra `LU-HEAVY-AMP` as the next bounded reviewed self-weapon amplification candidate. It must reuse the existing weapon damage-event proof/lifecycle (exact Lux & Umbra/rank, source actor, Echo-damage activation, ordering and independent per-build evidence) rather than invent a second trigger proof. Echo Skill amplification remains outside Character direct hits and DEF Ignore remains a separate arithmetic family. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
