...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `13fdee310a5d699778a97d3d8a55c1926b464a3d`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1205](https://github.com/bellebing/Bellibing-simulator/actions/runs/35312595859) and [Export #1104](https://github.com/bellebing/Bellibing-simulator/actions/runs/35312595835) passed on that exact runtime head with **924/924 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

The candidate source-qualifies partial non-Echo context for all 54 existing S0/max-skill Character hit consumers / 492 facts. In addition to the prior self/event families it now carries six explicit cross-owner ordinary-stat transfer capabilities: two Sonata Outro transfers, three Echo incoming transfers and Static Mist `STM-NEXT-ATK`. Every cross-owner contribution requires explicit source equipment plus actual source/recipient events independently per build; incoming Character presets and team membership never establish source gear or uptime.

Static Mist specifically requires a canonical released source Character, compatible verified `static-mist`, explicit R1-R5 source equipment evidence, actual Outro -> selected incoming Character identity and isolated lifecycle/query ordering. Rank values remain owned by the canonical weapon effect row, and duplicate same-effect source activations are rejected because stacking is unreviewed.

Exact Export artifact `10534455518` contains exactly one weapon incoming-transfer support row and independent reconstruction confirms **PARTIAL_L3 = 54** and **PARTIAL_L4 = 21**. This remains partial isolated-hit context only: complete L3–L7 stays two (Augusta/Ciaccona), readiness stays 43/3/9/2, all 83 edges / 72 dependency IDs and the six Reference Team blockers remain open. The next bounded ordinary-stat lane is cross-owner Rejuvenating Glow from explicit source 5-piece/team/heal evidence; amplification remains parked. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
