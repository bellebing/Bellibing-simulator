...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `ae4cb3806258f445a68a4d27144b75296be225b0`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1361](https://github.com/bellebing/Bellibing-simulator/actions/runs/35427565147) and [Export #1260](https://github.com/bellebing/Bellibing-simulator/actions/runs/35427565148) passed on that exact runtime head with **1066/1066 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

Checkpoint21 adds bounded Everbright Polestar negative-status DEF context. `EP-LIB-DEF` activates only from caller-qualified Fusion Burst or Tune Rupture - Shifting application evidence and requires exact Everbright Polestar R1-R5, source wielder, non-empty trigger target/source fact, explicit status qualification, isolated lifecycle and query ordering independently per build. Its eight-second source window is allowed to persist independently of the later hit target, but the DEF Ignore applies only to supported direct `LIBERATION` hits through the existing explicit enemy-DEF / no-hidden-defense-modifier boundary.

The sibling `EP-LIB-FUSION-RES` remains deliberately outside this primitive because RES-ignore arithmetic has not been reviewed. No status occurrence, mode, refresh, teammate equipment or target persistence is inferred. Exact Export artifact `10579437494` contains exactly one `weaponEverbrightPolestarDefenseHitContext` support row. Independent artifact reconstruction confirms **PARTIAL_L3 = 54** and **PARTIAL_L4 = 25**, with **Aemeath** as the only newly counted preset cohort; direct-hit facts remain 492.

Complete L3–L7 remains Augusta/Ciaccona only, readiness remains 43/3/9/2, and all 83 pending edges / 72 dependency IDs plus all six Reference Team blockers remain open. The next backend slice must be freshly reranked from actual remaining reviewed conditional families, preferring a single isolated window or already-structured prerequisite with direct Character-hit fan-out. Stack-history/mode/refresh families remain parked unless an explicit source-valid prerequisite exists. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
