...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `720ba472341b2f67ed3aa94e52e15289f73a1783`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1274](https://github.com/bellebing/Bellibing-simulator/actions/runs/35334479067) and [Export #1173](https://github.com/bellebing/Bellibing-simulator/actions/runs/35334479060) passed on that exact runtime head with **987/987 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

The candidate now carries typed source-qualified isolated-hit composition through ordinary stats, bounded cross-owner support, single-active scoped amplification and one bounded defense-arithmetic family. The existing direct-hit kernel remains unchanged. Scoped amplification classifies exact reviewed all-DMG / element / Character damage-class terms; zero or one applicable active term can populate the existing scalar, while multiple applicable terms are `PENDING_STACKING`. Character/Iuno Outros, Shorekeeper Binary Butterfly, Bloodpact's Pledge and Lux & Umbra `LU-HEAVY-AMP` all reuse their owning event/lifecycle primitives rather than inventing activation or uptime.

Lux & Umbra `LU-HEAVY-AMP` is activated only by source-qualified ECHO damage with exact selected weapon/rank and can affect only Character HEAVY hits. Echo Skill amplification stays outside ordinary Character direct-hit scope. Lethean Elegy `LE-DEF` is deliberately a separate defense contribution: exact ECHO-damage activation, selected Lethean/rank and per-build lifecycle/query proof are required, followed by explicit enemy DEF and proof that no other defense modifiers are hidden in the residual snapshot. The supplied baseline `defenseMultiplier` must equal the existing no-modifier defense kernel before the canonical R1-R5 DEF Ignore is applied. No generic DEF-ignore stacking policy is introduced.

Exact Export artifact `10542099832` contains exactly one `weaponDamageAmplificationHitContext` row (`LU-HEAVY-AMP`) and one `weaponDamageDefenseHitContext` row (`LE-DEF`). Independent artifact reconstruction confirms **PARTIAL_L3 = 54**, **PARTIAL_L4 = 21**, 492 direct-hit facts and two DPS_READY Characters. Complete L3–L7 therefore stays two (Augusta/Ciaccona), readiness remains 43/3/9/2, all 83 edges / 72 dependency IDs and all six Reference Team blockers remain open.

The next bounded candidate is Solsworn Ciphers `SCIP-AERO-DEF`: its reviewed 6-second SELF Aero-DMG DEF Ignore state opens from exact Echo Skill damage and can reuse the qualified damage-event plus explicit-defense boundary. It must remain Aero-hit-only and single-active for DEF-ignore arithmetic. Defier's Thorn / Blazing Justice remain source-semantics blocked, Thunderflare/Moongazer require stack history, Lux `LU-DEF` requires overlap state, and RES/target-debuff families require their own typed arithmetic boundary. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
