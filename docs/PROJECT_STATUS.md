...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `5d43b37dee2dd2a70394830ef7e8a10fb537ef2b`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1342](https://github.com/bellebing/Bellibing-simulator/actions/runs/35390633199) and [Export #1241](https://github.com/bellebing/Bellibing-simulator/actions/runs/35390633192) passed on that exact runtime head with **1051/1051 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

The candidate carries typed source-qualified isolated-hit composition through ordinary stats, bounded cross-owner support, scoped single-active amplification, explicit DEF-ignore/resistance arithmetic and current reviewed weapon status windows while leaving the core damage kernel unchanged. Checkpoint19 adds Forged Dwarf Star `FDS-LIB` only.

`FDS-LIB` activates from caller-qualified timestamped `FUSION_BURST_APPLIED` or `TUNE_STRAIN_SHIFTING_APPLIED` evidence with exact Forged Dwarf Star R1-R5, source wielder, trigger target, non-empty source fact, source qualification, isolated lifecycle and same-timestamp query ordering. Its canonical five-second Resonance Liberation DMG window is consumed only through the existing Liberation class-stat projection. The separate `FDS-TEAM` chained effect is not part of this primitive.

Checkpoint review also fixed an in-progress coverage error rather than promoting Character mechanics to fit the weapon. Denia recommends Forged Dwarf Star, but her current canonical Liberation-class ACTION facts are conditional and are intentionally excluded from `character-standard-explicit-hit-v1`; her supported direct-hit classes remain BASIC and HEAVY. The deterministic preset-reach audit now requires an actual supported LIBERATION direct hit before counting `FDS-LIB`.

Exact Export artifact `10566226584` contains exactly one `weaponForgedDwarfStarHitContext` row for `FDS-LIB`. Independent artifact reconstruction confirms **PARTIAL_L3 = 54** and **PARTIAL_L4 = 24**; without the corrected hit-class guard Denia would have been a false 25th cohort. Direct-hit coverage remains 492 facts, complete L3–L7 remains two Characters (Augusta/Ciaccona), readiness remains 43/3/9/2, and all 83 edges / 72 dependency IDs plus all six Reference Team blockers remain open.

The next bounded audit is the separate `FDS-TEAM` chained-state path. It may proceed only if current reviewed source semantics can represent: an already-active source-wielder `FDS-LIB` state, a selected teammate's own qualified Fusion Burst/Tune Strain - Shifting application, exact recipient/team identity, lifecycle/query ordering and same-name non-stacking without inferring Denia mode/rotation occurrence. Otherwise it stays parked and the workstream reranks. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
