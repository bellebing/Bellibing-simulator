...
- 33 characters have fully `VERIFIED` mechanics profiles;
...
- 24 released characters remain `UNSTARTED` for canonical Character mechanics promotion/source verification;
- 1068 canonical Character mechanic facts now exist across the 33 verified profiles, including exactly one current Tune Break fact per verified profile;
- all 33 verified profiles' `RAW_FACTS` preflight passes only when their canonical structural audit is clean, while independent build/team/rotation/combat-profile requirements continue to gate later stages.
...
- audit and promote source-backed skill/Forte/passive/resource/Outro/sequence semantics for the remaining 24 released characters using the promotion-review artifacts rather than hand-entering source tables or description numerics;
