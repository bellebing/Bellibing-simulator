...
- 48 characters have fully `VERIFIED` mechanics profiles;
...
- 9 released characters remain `UNSTARTED` for canonical Character mechanics promotion/source verification;
- 1623 canonical Character mechanic facts now exist across the 48 verified profiles, including exactly one current Tune Break fact per verified profile;
...
- Luuk Herssen remains unpromoted pending full semantic re-audit; PR #81 now provides a truthful fixed-flat raw representation, but schema availability alone does not verify his profile;
...
7. **CURRENT RETURN CHECKPOINT:** Review all remaining 9 Character Mechanics profiles together from the PR #66/#68 artifacts plus current sources: Buling, Danjin, Lucy, Luuk Herssen, Rebecca, Rover (Electro), Suisui, Xiangli Yao and Zani. Promote only profiles whose source/schema/import blockers are truthfully resolved; preserve contradictions or unstated/multi-class semantics explicitly rather than normalizing them. Do not begin broad Character DPS, Echo/Sonata, UI or Roll/Stop optimization while released-roster Character Mechanics coverage remains open.
