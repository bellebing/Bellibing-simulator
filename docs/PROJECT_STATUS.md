...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `ac7fd5ec4941c6f72393da3fcc2906981f35aa77`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1286](https://github.com/bellebing/Bellibing-simulator/actions/runs/35335698019) and [Export #1185](https://github.com/bellebing/Bellibing-simulator/actions/runs/35335698014) passed on that exact runtime head with **1000/1000 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

The candidate carries typed source-qualified isolated-hit composition through ordinary stats, bounded cross-owner support, single-active scoped amplification and bounded single-active DEF-ignore arithmetic while leaving the existing damage kernel unchanged. Character/Iuno Outros, Shorekeeper Binary Butterfly, Bloodpact's Pledge and Lux & Umbra `LU-HEAVY-AMP` feed the typed amplification boundary only from their owning event/lifecycle proof; multiple applicable terms remain `PENDING_STACKING`.

Defense arithmetic currently consumes three bounded reviewed paths. Lethean Elegy `LE-DEF` reuses exact ECHO-damage activation; Solsworn Ciphers `SCIP-AERO-DEF` reuses exact ECHO damage but applies only to explicit Aero Character hits; Lux & Umbra `LU-DEF` exists only while both exact reviewed Lux amplification windows overlap for the same wielder. Every active DEF-ignore path requires explicit enemy DEF, proof that no other defense modifiers are hidden in the residual snapshot, and a caller baseline `defenseMultiplier` equal to the existing no-modifier defense kernel before canonical R1-R5 DEF Ignore is applied. Multiple active DEF-ignore terms remain unsupported stacking.

Exact Export artifact `10542786560` is bound to `ac7fd5ec4941c6f72393da3fcc2906981f35aa77`. **PARTIAL_L3 = 54**, **PARTIAL_L4 = 21**, direct-hit facts remain 492 and only Augusta/Ciaccona remain DPS_READY. Readiness remains 43/3/9/2; all 83 edges / 72 dependency IDs and all six Reference Team blockers remain open.

The next bounded arithmetic candidate is Woodland Aria `WA-AERO-RES`. It must use a separate typed resistance boundary rather than DEF arithmetic: exact selected Woodland Aria/R1-R5, source-qualified hit of the same explicit target while pre-trigger Aero Erosion is proven, explicit selected-target identity, target base resistance, proof that no other resistance modifiers are hidden in the residual multiplier, isolated lifecycle/query ordering and per-build evidence. No RES Ignore, target DMG Taken, multi-resistance stacking or profile uptime may be inferred. Spectral Trigger remains parked until exact Hack - Shifting event semantics exist; Thunderflare/Moongazer remain stack-history lanes; Defier's Thorn/Blazing Justice remain source-semantics blocked. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
