...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `9d333017195c27367b436a4e1178c3c3353b60d2`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1379](https://github.com/bellebing/Bellibing-simulator/actions/runs/35432623320) and [Export #1278](https://github.com/bellebing/Bellibing-simulator/actions/runs/35432623321) passed on that exact runtime head with **1082/1082 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

Checkpoint23 adds bounded Red Spring Concerto context. `RS-CONCERTO-BASIC` activates only from caller-qualified `CONCERTO_ENERGY_CONSUMED` evidence with exact Red Spring R1–R5, source wielder/fact, explicit source qualification, canonical cooldown-ready state, isolated lifecycle, full switch-out history and trigger/switch query ordering independently per build. The canonical 10-second Basic Attack DMG window ends when the wielder switches off field. The separate `RS-BASIC` stacking family remains pending; Concerto proof neither creates nor clears that stack state.

The initial Red Spring candidate had two regression-fixture defects only: a nonexistent Camellya SKILL direct-hit fixture and an unreachable specialized refresh error behind the generic duplicate-effect guard. The exact green head uses Rover (Havoc)'s canonical SKILL hit for the non-BASIC leakage test and prioritizes the dedicated repeated-Concerto fail-closed guard. No gameplay value or source fact changed.

Exact Export artifact `10581670265` contains exactly one `weaponRedSpringConcertoHitContext` row. Independent artifact reconstruction confirms **PARTIAL_L3 = 54** and **PARTIAL_L4 = 27**, up from 25: **Camellya** and **Rover (Havoc)** are the two newly counted preset cohorts. Direct-hit facts remain 492. Complete L3–L7 remains Augusta/Ciaccona only, readiness remains 43/3/9/2, and all 83 pending edges / 72 dependency IDs plus all six Reference Team blockers remain open.

Fresh rerank identifies the shared **Hack - Shifting** family as the next bounded source-valid lane. Spectral Trigger `SPT-HEAVY-AMP` / `SPT-HEAVY-DEF` and Skull Thrasher `SKT-HACK-BASIC` / `SKT-HACK-TEAM` have explicit current source text and can reuse existing typed amplification, DEF and ordinary-stat boundaries from a caller-qualified status application. `SPT-SPECTRO` stays parked because its two-stack cast history is unreviewed. Wildfire Mark `WM-FUSION` stays parked despite wider preset reach because its canonical row still exposes only the vague legacy trigger “Extended team effect” with no current sourceEffectText sufficient to bind occurrence. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
