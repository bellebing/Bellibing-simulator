...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `cd62fe9c22b2ef9e81296d69bb69102b25f4dd31`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1311](https://github.com/bellebing/Bellibing-simulator/actions/runs/35346779894) and [Export #1210](https://github.com/bellebing/Bellibing-simulator/actions/runs/35346779876) passed on that exact runtime head with **1020/1020 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

The candidate carries typed source-qualified isolated-hit composition through ordinary stats, bounded cross-owner support, single-active scoped amplification, bounded single-active DEF-ignore arithmetic and a separate resistance-reduction boundary while leaving the existing damage kernel unchanged. Woodland Aria now has two independent timestamp/evidence paths: `WA-AERO-RES` for same-target Aero RES Reduction after a pre-trigger Aero Erosion observation, and `WA-AERO` for the wielder's 10-second Aero DMG window after an explicitly caller-qualified `AERO_EROSION_APPLIED` event. The latter never converts the historical event-indexed rotation adapter to time and never parses Character prose to infer application occurrence.

The new cooldown-qualified cast family adds exactly five canonical ATK% windows: `CS-ATK`, `EC-ATK`, `FA-ATK`, `RJ-ATK` and `WR-ATK`. Each requires exact selected weapon R1-R5, exact wielder/Resonance Skill cast, caller-proven cooldown readiness at the event, isolated prior state, no later activation through the hit and explicit query ordering. Canonical 16-second duration, 20-second trigger cooldown and rank values remain in the weapon rows. The companion resource effects stay separate; proving an ATK window does not award resource.

Exact Export artifact `10547895585` contains one `weaponStatusApplicationHitContext` row for `WA-AERO`, five `weaponCooldownCastHitContext` rows and the existing `WA-AERO-RES` target-resistance row. Independent artifact reconstruction confirms **PARTIAL_L3 = 54**, **PARTIAL_L4 = 22**, 492 direct-hit facts and only Augusta/Ciaccona DPS_READY. `WA-AERO` reuses the already-counted Ciaccona Woodland Aria cohort and the cooldown batch reaches already-counted Augusta via Waning Redshift, so neither creates false additional cohort count. Readiness remains 43/3/9/2; all 83 edges / 72 dependency IDs and all six Reference Team blockers remain open.

The next bounded candidate is Freeze Frame's paired `FF-GLACIO` SELF Glacio-DMG and `FF-TEAM-ATK` TEAM ATK windows after an explicit Glacio Chafe application. No typed application primitive exists yet; any implementation must use caller-qualified timestamped application evidence with exact source weapon/rank/actor/target/source fact, keep the two canonical windows separate, require explicit selected-team membership for the TEAM effect and reject unreviewed same-name stacking. Lucilla's Freeze Frame preset is not yet counted in PARTIAL_L4, so this lane can add a real cohort without relying on stack history. Spectral Trigger/Hack stays parked, Thunderflare/Moongazer remain stack-history lanes, and Defier's Thorn/Blazing Justice remain source-semantics blocked. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
