...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `93fe67df24b8ad6717daee5b8fe032fefe308e1c`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1332](https://github.com/bellebing/Bellibing-simulator/actions/runs/35365637286) and [Export #1231](https://github.com/bellebing/Bellibing-simulator/actions/runs/35365637285) passed on that exact runtime head with **1043/1043 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

The candidate carries typed source-qualified isolated-hit composition through ordinary stats, bounded cross-owner support, single-active scoped amplification, bounded single-active DEF-ignore arithmetic and a separate resistance-reduction boundary while leaving the existing damage kernel unchanged. Checkpoint18 adds Azure Oath's paired Havoc-Bane-triggered Heavy windows using a source-reviewed caller-qualified status occurrence rather than Character-prose or rotation inference.

Current Azure Oath sources explicitly state that its conditional effect starts **after inflicting Havoc Bane**. Runtime therefore requires an exact timestamped `HAVOC_BANE_APPLIED` occurrence with Azure Oath R1-R5, source actor, target, non-empty source fact, positive applied stacks, source qualification, isolated lifecycle and query ordering. `AO-HEAVY-AMP` feeds the existing single-active amplification boundary only for HEAVY Character hits. `AO-DEF` feeds the existing explicit enemy-DEF/no-hidden-modifier boundary only for HEAVY Character hits; Basic/non-Heavy hits observe the source window as non-applicable rather than inheriting DEF Ignore.

Exact Export artifact `10555748328` contains one `weaponAzureOathAmplificationHitContext` row and one `weaponAzureOathDefenseHitContext` row. Independent reconstruction confirms **PARTIAL_L3 = 54** and **PARTIAL_L4 = 24**, with **Yangyang: Xuanling as the only new checkpoint18 cohort** through her exact reviewed Azure Oath recommendation. Direct-hit coverage remains 492 facts, complete L3–L7 remains two (Augusta/Ciaccona), readiness stays 43/3/9/2, and all 83 edges / 72 dependency IDs plus all six Reference Team blockers remain open.

The next bounded candidate is Forged Dwarf Star `FDS-LIB` only. Current sources explicitly bind the wielder's 5-second Resonance Liberation DMG window to the wielder inflicting Fusion Burst or Tune Strain - Shifting. Any implementation must use caller-qualified timestamped status-application evidence and must not infer occurrence from Denia's source sequence. The separate `FDS-TEAM` effect remains parked behind its chained prerequisite: the self window must already be active before a team Resonator's own status application can grant that Resonator ATK, with same-name non-stacking preserved. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
