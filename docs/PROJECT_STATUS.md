...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `720baf97790614a3d24b9b62dbb9971d5b933391`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1230](https://github.com/bellebing/Bellibing-simulator/actions/runs/35317271836) and [Export #1129](https://github.com/bellebing/Bellibing-simulator/actions/runs/35317271784) passed on that exact runtime head with **942/942 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

The candidate source-qualifies partial non-Echo context for all 54 existing S0/max-skill Character hit consumers / 492 facts. Its bounded cross-owner ordinary-stat layer now includes two Sonata Outro transfers, three Echo incoming transfers, Static Mist, Rejuvenating Glow, Stellar Symphony and Fallacy team ATK. Every cross-owner contribution requires explicit canonical source identity/equipment plus the exact source event/team/recipient proof independently per build; incoming Character presets and team membership never establish source gear or uptime.

The Fallacy bridge preserves the existing semantic split: only `FALLACY_TEAM_ATK = ATK%` is exposed as a team hit-context capability. `FALLACY_WIELDER_ER = Energy Regen` remains wielder-only. The source must be a canonical released Character with Fallacy as explicit main Echo, the selected canonical team must include source and recipient, and the exact `ECHO_SKILL_CAST` plus lifecycle/query ordering must be provided separately per build. The cross-owner path cannot substitute for a selected Character's own equipment path.

Exact Export artifact `10536032527` contains exactly one `echoTeamStatHitContext` support row. Cross-owner/teamgear capabilities remain excluded from preset-derived reach, so **PARTIAL_L3 = 54** and **PARTIAL_L4 = 21**. Complete L3–L7 stays two (Augusta/Ciaccona), readiness stays 43/3/9/2, all 83 edges / 72 dependency IDs and the six Reference Team blockers remain open. In particular, `shorekeeper-fallacy-team-atk-augusta-window-overlap` is not closed by the generic bridge because no real Reference Team timestamp/overlap proof was added.

The currently reviewed ordinary-stat cross-owner lane is now exhausted. Remaining nearby reusable primitives are amplification/state lanes (Shorekeeper Outro, Bloodpact's Pledge, Iuno/Character Outros) whose stacking and overlap semantics cannot be collapsed into the generic hit `amplification` field without an explicit scoped-composition contract and regression suite. This is the next integration/semantics boundary, not a missing adapter. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
