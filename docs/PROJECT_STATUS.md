...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `db5186ffe66994df2c0424de3fd6ef1603e67563`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1246](https://github.com/bellebing/Bellibing-simulator/actions/runs/35329672131) and [Export #1145](https://github.com/bellebing/Bellibing-simulator/actions/runs/35329672132) passed on that exact runtime head with **960/960 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

The candidate source-qualifies partial non-Echo context for all 54 existing S0/max-skill Character hit consumers / 492 facts. The previously completed ordinary-stat layer remains unchanged; amplification now has a separate typed composition boundary rather than being coerced into the generic stat path. Exact reviewed labels are classified as all-damage, element or Character damage-class scope. The existing damage kernel is unchanged.

The amplification contract deliberately supports **zero or one active applicable term**. More than one active applicable term is `PENDING_STACKING`; it is never summed or multiplied. `Echo Skill DMG Amplification` remains outside Character direct-hit scope. If one source-qualified amplification term is active, the residual caller `amplification` value must be zero, preventing unreviewed cross-source double counting.

Two source-qualified amplification families now consume that boundary. Shorekeeper Binary Butterfly requires exact Shorekeeper Outro, selected canonical team membership, isolated lifecycle and per-build ordering. Bloodpact's Pledge `BPP-TEAM-AERO` requires canonical Rover (Aero), verified Bloodpact R1–R5 source equipment, exact Unbound Flow cast and explicit nearby/on-field recipient eligibility. A simultaneous Shorekeeper all-DMG plus Bloodpact Aero hit is regression-locked to `PENDING_STACKING`.

Exact Export artifact `10540796176` contains both `shorekeeperTeamAmplification` and `weaponTeamAmplificationHitContext` support surfaces while canonical BPP source data remains unchanged. Cross-owner/teamgear capabilities remain excluded from preset-derived reach, so **PARTIAL_L3 = 54** and **PARTIAL_L4 = 21**. Complete L3–L7 stays two (Augusta/Ciaccona), readiness stays 43/3/9/2, all 83 edges / 72 dependency IDs and all six Reference Team blockers remain open.

The next bounded candidate lane is Character/Iuno Outro amplification under the same single-active rule. Any bridge must preserve actual incoming recipient identity, explicit switch-out history and same-timestamp query ordering. Dual-term source Outros may execute only where exactly one term applies to the selected hit; two applicable terms stay pending. Generic primitive availability cannot close profile overlap dependencies without real timeline evidence. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
