...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `515b375de3f4ff5c4d3bd11f3649df6c85a70a79`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1222](https://github.com/bellebing/Bellibing-simulator/actions/runs/35313733105) and [Export #1121](https://github.com/bellebing/Bellibing-simulator/actions/runs/35313733094) passed on that exact runtime head with **936/936 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

The candidate source-qualifies partial non-Echo context for all 54 existing S0/max-skill Character hit consumers / 492 facts. Its bounded cross-owner ordinary-stat layer now includes two Sonata Outro transfers, three Echo incoming transfers, Static Mist, Rejuvenating Glow and Stellar Symphony. Every cross-owner contribution requires explicit canonical source identity/equipment plus the exact source event/team/recipient proof independently per build; incoming Character presets and team membership never establish source gear or uptime.

Rejuvenating Glow `REJUV_ATK` requires exact source 5-piece Sonata-7, selected canonical team membership and a source-qualified applied-heal event. Stellar Symphony `SSY-TEAM-ATK` requires The Shorekeeper, verified Stellar Symphony R1-R5 equipment, the canonical Chaos Theory healing Skill fact and selected canonical team membership. Self/cross-owner Rejuvenating Glow double counting and duplicate same-effect sources are rejected because stacking is unreviewed.

Exact Export artifact `10534262863` contains one `sonataTeamHealHitContext` support row and one `weaponTeamStatHitContext` support row. Cross-owner/teamgear capabilities remain excluded from preset-derived reach, so **PARTIAL_L3 = 54** and **PARTIAL_L4 = 21**. Complete L3–L7 stays two (Augusta/Ciaccona), readiness stays 43/3/9/2, all 83 edges / 72 dependency IDs and the six Reference Team blockers remain open.

The next bounded ordinary-stat lane is `FALLACY_TEAM_ATK` only. `FALLACY_WIELDER_ER` stays wielder-only, and the generic team-stat bridge must not close the Reference Team Fallacy/Augusta overlap blocker without real profile timing/overlap proof. Character Outro, Shorekeeper Outro and weapon-team amplification remain outside the generic hit context until scoped amplification stacking/overlap semantics are proven. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
