...
The active UI lane now has a separate draft runtime candidate, **PR #208**, built from current canonical main after the approved PR #207 feel/card prototype. PR #208 moves the shared freewheel carousel into the real `v34-functional.html` preview for Home + Build/Improve, reads the canonical Character portrait manifest for exactly **57 released selector cards**, keeps names header-first and portraits uncropped, and establishes a finite centered **1440px AppShell**. Draft PR #205 remains a separate Character-identity/Sequence candidate requiring its own user review/merge decision; PR #208 does not silently subsume it.

PR #208 runtime checkpoint `ade841d178c12609ff252ccf0ea063a6052334ec` is exact-head green: Verify run `36022021673`, Export run `36022021544`, and Import run `36022021727` all completed successfully with **1096/1096 tests, zero failures**. The new real-Chrome gate covers 390×844, 768×1024, 1440×900, 1920×1080, 2560×1440, 3440×1440 and 7680×2160; it verifies the centered finite AppShell, no horizontal page scroll, Home's fixed three-card/default-Improve contract, 57/57 released Character portraits, header/framing rules, release filtering, multi-card freewheel drag and centered activation. Desktop drag verified 3/57→13/57; mobile verified 3/57→6/57. Visual artifact `10817171925` was reviewed at phone, desktop and 7680px; the ultrawide carousel is clipped to the finite application shell rather than leaking cards toward the monitor edges.

The responsive gate also exposed a real mobile layout defect during development: the hidden pre-selection `.build-stage` remained in flex flow under the old mobile CSS and pushed the Character wheel above the viewport. PR #208 now keeps that hidden stage out of flow until Character selection; the corrected mobile wheel is browser-verified. This is candidate evidence only: PR #208 is still **OPEN / DRAFT / UNMERGED**, and no live deployment claim follows. **BUG-030 remains open** because deeper Stats/Weapon/Echo/skills wiring, approved hero focal derivatives, mobile progressive-disclosure drawers and final live parity are still incomplete. Canonical implementation rules remain in `docs/UI_LAYOUT_MOTION_CONTRACT.md`.
