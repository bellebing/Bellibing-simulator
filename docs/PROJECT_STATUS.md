...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `16adbb0d937cc335844866d9346aa9d411f830cb`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1196](https://github.com/bellebing/Bellibing-simulator/actions/runs/35312106001) and [Export #1095](https://github.com/bellebing/Bellibing-simulator/actions/runs/35312106003) passed on that exact runtime head with **918/918 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

The candidate source-qualifies partial non-Echo context for all 54 existing S0/max-skill Character hit consumers / 492 facts: level-90 Character/core weapon stats, max Minor Fortes, 60 permanent weapon effects, 30 static Sonata effects, 50 reviewed static main-Echo effects, weapon cast/damage/heal windows, Sonata cast/Rejuvenating Glow healing, bounded Sonata damage, explicit target-qualified Sonata windows, two cross-owner Sonata Outro transfers and three cross-owner Echo incoming transfers.

Cross-owner transfer composition never derives source teammate gear from the incoming Character preset. Sonata requires explicit source 5-piece equipment; Echo transfers require explicit source main-Echo/equipment plus any source-required rank, exact arm event, Outro/incoming identity/entry mode, isolated lifecycle and hit-time ordering per build. Voidwing Moth retains Rank-5/use-to-Outro semantics; Hyvatia retains its incoming Intro requirement. Duplicate same-effect activations are rejected because stacking is unreviewed.

Exact Export artifact `10533294801` contains the three Echo incoming-transfer support rows and independent reconstruction confirms **PARTIAL_L3 = 54** and **PARTIAL_L4 = 21**. This remains partial isolated-hit context only: complete L3–L7 stays two (Augusta/Ciaccona), readiness stays 43/3/9/2, all 83 edges / 72 dependency IDs and the six Reference Team blockers remain open. Next work must rerank already-reviewed ordinary cross-owner team/stat windows before any amplification integration; Character Outro and weapon team amplification remain parked until scoped stacking/overlap semantics are proven. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
