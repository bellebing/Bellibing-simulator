...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `feb6f0924fcca4e5d987b8a7abe517ad7c131ffe`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1177](https://github.com/bellebing/Bellibing-simulator/actions/runs/35309732718) and [Export #1076](https://github.com/bellebing/Bellibing-simulator/actions/runs/35309732761) passed on that exact runtime head with **905/905 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

The candidate source-qualifies partial non-Echo context for all 54 existing S0/max-skill Character hit consumers / 492 facts: level-90 Character/core weapon stats, max Minor Fortes, 60 permanent weapon effects, 30 static Sonata effects, 50 reviewed static main-Echo effects, weapon cast/damage/heal windows, Sonata cast/Rejuvenating Glow healing, bounded Sonata damage, and explicit target-qualified Sonata windows. Current and candidate builds retain independent equipment/event proof; missing event/state/source scope remains named PENDING rather than zero.

The Sonata damage bridge consumes only `S22_3PC_HEAVY_CR` for selected HEAVY direct hits and `S29_5PC_AERO` as Aero DMG Bonus; the two Echo Skill CRIT Rate windows remain outside ordinary Character-hit context. Target-qualified Sonata composition consumes `S11_5PC_SPECTRO`, `S17_5PC_CR` and `S17_5PC_AERO` only after exact pre-trigger target state, source-qualified attack/hit occurrence, owner/equipment and query ordering are proven per build. Exact Export inspection reports **PARTIAL_L3 = 54** and **PARTIAL_L4 = 21**; target context newly reaches Cartethyia, Jiyan, Rover (Aero) and Zani in addition to the prior Galbrena/Sigrika Sonata-damage cohorts.

This remains partial isolated-hit context only: complete L3–L7 stays two (Augusta/Ciaccona), readiness stays 43/3/9/2, all 83 edges / 72 dependency IDs and the six Reference Team blockers remain open. Character Outro amplification remains outside the generic hit context until scoped stacking/overlap semantics are proven. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
