...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `2a4ec99fc00614482059349c41f32e34a1ec140b`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1322](https://github.com/bellebing/Bellibing-simulator/actions/runs/35364082427) and [Export #1221](https://github.com/bellebing/Bellibing-simulator/actions/runs/35364082371) passed on that exact runtime head with **1032/1032 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

The candidate carries typed source-qualified isolated-hit composition through ordinary stats, bounded cross-owner support, single-active scoped amplification, bounded single-active DEF-ignore arithmetic and a separate resistance-reduction boundary while leaving the existing damage kernel unchanged. Checkpoint17 adds an explicit caller-qualified Glacio Chafe application boundary for Freeze Frame without converting source prose, source rotations or teammate identity into occurrence/uptime.

Freeze Frame keeps its two canonical windows separate: `FF-GLACIO` is SELF Glacio DMG and `FF-TEAM-ATK` is TEAM ATK%. Exact Freeze Frame R1-R5, source actor, trigger target, non-empty source fact, positive applied stacks, timestamp/source qualification, isolated lifecycle and query ordering are required independently per build. Team composition additionally requires an explicit selected canonical team. Cross-owner composition exports only `FF-TEAM-ATK`; the wielder-only Glacio window never leaks to teammates. Own plus cross-owner same-name TEAM ATK is rejected because stacking is unreviewed.

Exact Export artifact `10555795453` contains exactly two `weaponFreezeFrameHitContext` rows. Independent reconstruction confirms **PARTIAL_L3 = 54** and **PARTIAL_L4 = 23**, with **Lucilla as the only new checkpoint17 preset cohort** through her exact reviewed Freeze Frame recommendation. Cross-owner/teamgear capability does not create inferred preset reach. Direct-hit coverage remains 492 facts, complete L3–L7 remains two (Augusta/Ciaccona), readiness stays 43/3/9/2, and all 83 edges / 72 dependency IDs plus all six Reference Team blockers remain open.

The next candidate is Azure Oath's paired `AO-HEAVY-AMP` / `AO-DEF` Havoc-Bane-triggered window for Yangyang: Xuanling. It may continue only if current reviewed canonical sources justify a caller-qualified timestamped Havoc Bane application occurrence without parsing Character prose or promoting RAW_ONLY stack mechanics. Amplification must reuse the existing single-active scoped contract and DEF Ignore must reuse the explicit enemy-DEF/no-hidden-modifier boundary. If that source boundary is insufficient, Azure Oath remains pending and the lane is reranked. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
