...
Draft PR #198 remains **OPEN / UNMERGED** on `codex/source-qualified-combat-context-2026-09-15`. Its latest fully verified runtime checkpoint is `ace8d701580f12d99167e8eadb3db4c275b3c4c4`, based on canonical main `3ce9b150a7a5b1f4bc8947f74411d6df9ac859bf`. [Verify #1296](https://github.com/bellebing/Bellibing-simulator/actions/runs/35337534126) and [Export #1195](https://github.com/bellebing/Bellibing-simulator/actions/runs/35337534233) passed on that exact runtime head with **1007/1007 tests, zero failures**, all deterministic source/profile/readiness gates, strict build, real-Chrome regressions and whitespace checks.

The candidate carries typed source-qualified isolated-hit composition through ordinary stats, bounded cross-owner support, single-active scoped amplification, bounded single-active DEF-ignore arithmetic and now a separate single-active resistance-reduction boundary while leaving the existing damage kernel unchanged. Character/Iuno Outros, Shorekeeper Binary Butterfly, Bloodpact's Pledge and Lux & Umbra `LU-HEAVY-AMP` feed typed amplification only from owning event/lifecycle proof; multiple applicable terms remain `PENDING_STACKING`. Lethean Elegy, Solsworn Ciphers and Lux overlap feed bounded DEF-ignore only with explicit enemy DEF, canonical no-modifier baseline validation and no-hidden-defense-modifier proof.

Woodland Aria `WA-AERO-RES` now uses its own reviewed target-window contract. Exact selected Woodland Aria R1-R5, a source-qualified `HIT_TARGET` on the same explicit selected target, pre-trigger Aero Erosion state, isolated lifecycle/query ordering, explicit target base resistance and proof that no other resistance modifiers are hidden in the residual multiplier are required independently per build. It affects only explicit Aero Character hits; no RES Ignore, target DMG Taken or multi-resistance stacking is inferred.

Exact Export artifact `10544105330` contains exactly one `weaponTargetResistanceHitContext` row for `WA-AERO-RES`. Independent artifact reconstruction confirms **PARTIAL_L3 = 54** and **PARTIAL_L4 = 22** (up from 21 because Ciaccona's Woodland Aria preset now has a real supported event-context bridge), with 492 direct-hit facts and only Augusta/Ciaccona DPS_READY. Readiness remains 43/3/9/2; all 83 edges / 72 dependency IDs and all six Reference Team blockers remain open.

The next adjacent candidate is Woodland Aria `WA-AERO`, but its older reviewed adapter is event-indexed for fixed short rotations while PR198 hit context is timestamp/evidence based. No event-index-to-time conversion is allowed. A continuation may only use an explicit timestamped, caller-qualified Aero Erosion application event with exact weapon/owner/target/source-fact identity and ordering; otherwise the lane stays parked. Spectral Trigger remains blocked on exact Hack - Shifting event semantics, Thunderflare/Moongazer remain stack-history lanes, and Defier's Thorn/Blazing Justice remain source-semantics blocked. No merge is authorized. See [the combat-context campaign](COMBAT_CONTEXT_CAMPAIGN_20260915.md).
