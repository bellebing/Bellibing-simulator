...
- A07: resolved team manifests are detached snapshots of registry selection and caller dependencies. A candidate's nested Sonata selection or contribution edits cannot retarget canonical data, another candidate, or a later resolution; subsequent caller edits cannot rewrite an existing manifest. Mutation-isolation regressions preserve six pending dependencies and `dpsReady=false`.
