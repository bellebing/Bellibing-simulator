...
async function verifyWeaponOverlay(send, width, height, capturePath) {
  ...
  if(width>760&&(Math.abs(fresh.button.width-176)>1.5||Math.abs(fresh.button.height-176)>1.5)) throw new Error(`Desktop Build Weapon slot must be square 176x176: ${JSON.stringify(fresh.button)}`);
  ...
  const firstActive=await evaluate(send,`(()=>{const choices=[...document.querySelectorAll('#weaponChoices .weapon-choice')],first=choices[0],equip=document.getElementById('weaponEquip'),slot=document.querySelector('#weaponSlotHost .weapon-card'),badge=first?.querySelector('.weapon-equipped-badge');return{currentId:weaponUi.currentId,previewId:weaponUi.previewId,order:choices.map(x=>x.dataset.weaponId),firstId:first?.dataset.weaponId||null,firstEquipped:first?.classList.contains('is-equipped')||false,badgeText:badge?.textContent.trim()||null,badgeDisplay:badge?getComputedStyle(badge).display:null,slotId:slot?.dataset.weaponId||null,equipText:equip.textContent.trim(),equipDisabled:equip.disabled,flying:document.querySelectorAll('.weapon-card.is-flying').length}})()`);
  if(firstActive.currentId!=='preview-01'||firstActive.previewId!=='preview-01'||firstActive.firstId!=='preview-01'||!firstActive.firstEquipped||firstActive.badgeText!=='Active Weapon'||firstActive.badgeDisplay==='none'||firstActive.slotId!=='preview-01'||firstActive.equipText!=='Active Weapon'||!firstActive.equipDisabled||firstActive.flying!==0||JSON.stringify(firstActive.order)!==JSON.stringify(baseOrder)) throw new Error(`Explicit Equip did not establish Active slot 1: ${JSON.stringify(firstActive)}`);
  ...
  if(touch){
    ...
    if(mobileClosed.mounted||mobileClosed.hidden!=='true'||mobileClosed.currentId!=='preview-01'||mobileClosed.slotId!=='preview-01'||mobileClosed.flying!==0) throw new Error(`Mobile Weapon preview/equip smoke close failed: ${JSON.stringify(mobileClosed)}`);
  ...
  for(const id of ['preview-02','preview-03','preview-04','preview-06']){
    ...
    const state=await evaluate(send,`(()=>({currentId:weaponUi.currentId,previewId:weaponUi.previewId,order:[...document.querySelectorAll('#weaponChoices .weapon-choice')].map(x=>x.dataset.weaponId),firstId:document.querySelector('#weaponChoices .weapon-choice')?.dataset.weaponId||null,slotId:document.querySelector('#weaponSlotHost .weapon-card')?.dataset.weaponId||null,equipText:document.getElementById('weaponEquip').textContent.trim(),equipDisabled:document.getElementById('weaponEquip').disabled,flying:document.querySelectorAll('.weapon-card.is-flying').length}))()`);
    if(state.currentId!=='preview-01'||state.previewId!==id||state.firstId!=='preview-01'||state.slotId!=='preview-01'||state.equipText!=='Equip Weapon'||state.equipDisabled||state.flying!==0||JSON.stringify(state.order)!==JSON.stringify(baseOrder)) throw new Error(`Preview browsing mutated Active/grid for ${id}: ${JSON.stringify(state)}`);
  }
  ...
  if(!specOk||!traceOk||tunnel.currentId!=='preview-01'||tunnel.previewId!=='preview-02'||tunnel.firstId!=='preview-01'||tunnel.slotId!=='preview-01'||tunnel.flying!==0||tunnel.gridCards!==7) throw new Error(`Weapon Preview tunnel changed Active/grid state: ${JSON.stringify(tunnel)}`);
  ...
  const committed=await evaluate(send,`(()=>{const choices=[...document.querySelectorAll('#weaponChoices .weapon-choice')],first=choices[0],second=choices[1],equip=document.getElementById('weaponEquip'),slot=document.querySelector('#weaponSlotHost .weapon-card');return{currentId:weaponUi.currentId,previewId:weaponUi.previewId,order:choices.map(x=>x.dataset.weaponId),firstId:first?.dataset.weaponId||null,firstEquipped:first?.classList.contains('is-equipped')||false,secondId:second?.dataset.weaponId||null,slotId:slot?.dataset.weaponId||null,equipText:equip.textContent.trim(),equipDisabled:equip.disabled,flying:document.querySelectorAll('.weapon-card.is-flying').length}})()`);
  const expected=['preview-02','preview-01','preview-03','preview-04','preview-05','preview-06','preview-07'];
  if(committed.currentId!=='preview-02'||committed.previewId!=='preview-02'||committed.firstId!=='preview-02'||!committed.firstEquipped||committed.secondId!=='preview-01'||committed.slotId!=='preview-02'||committed.equipText!=='Active Weapon'||!committed.equipDisabled||committed.flying!==0||JSON.stringify(committed.order)!==JSON.stringify(expected)) throw new Error(`Equip commit did not perform Active slot 1 swap correctly: ${JSON.stringify(committed)}`);
  ...
  const closing=await evaluate(send,`(()=>{const o=document.getElementById('weaponOverlay');return{mounted:o.classList.contains('mounted'),open:o.classList.contains('open'),hidden:o.getAttribute('aria-hidden'),currentId:weaponUi.currentId,slotId:document.querySelector('#weaponSlotHost .weapon-card')?.dataset.weaponId||null,flying:document.querySelectorAll('.weapon-card.is-flying').length}})()`);
  if(!closing.mounted||closing.open||closing.hidden!=='false'||closing.currentId!=='preview-02'||closing.slotId!=='preview-02'||closing.flying!==0) throw new Error(`Weapon close must preserve committed Active without card flights: ${JSON.stringify(closing)}`);
  await sleep(360);
...
try{
  ...
  try{
    ...
    console.log(`- Weapon Preview/Equip separation passed with square 176x176 desktop Build slot and centered primary labels; detailed stat/effect text remains left-aligned; desktop equipped: ${desktopWeapon.equipped}, mobile smoke equipped: ${mobileWeapon.equipped}.`);
