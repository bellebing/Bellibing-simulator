...
async function verifyWeaponOverlay(send, width, height, capturePath) {
  ...
  await sleep(470);
  const opened=await evaluate(send,`(()=>{const o=document.getElementById('weaponOverlay'),p=document.getElementById('weaponPanel'),scrim=document.querySelector('.weapon-scrim'),browser=document.getElementById('weaponBrowser'),rail=document.getElementById('weaponScrollRail'),thumb=rail?.querySelector('.weapon-scroll-thumb'),r=p.getBoundingClientRect(),ps=getComputedStyle(p),ss=getComputedStyle(scrim),equip=document.getElementById('weaponEquip'),es=getComputedStyle(equip),bs=getComputedStyle(browser),ts=thumb?getComputedStyle(thumb):null;return{mounted:o.classList.contains('mounted'),open:o.classList.contains('open'),hidden:o.getAttribute('aria-hidden'),expanded:document.getElementById('weaponBtn').getAttribute('aria-expanded'),options:document.querySelectorAll('#weaponChoices .weapon-choice').length,order:[...document.querySelectorAll('#weaponChoices .weapon-choice')].map(x=>x.dataset.weaponId),equipped:document.querySelectorAll('#weaponChoices .weapon-choice.is-equipped').length,previewLayers:document.querySelectorAll('#weaponPreviewStage .weapon-preview-layer').length,previewHidden:document.getElementById('weaponPreviewPane').getAttribute('aria-hidden'),equipText:equip?.textContent.trim()||null,equipDisabled:equip?.disabled??null,flying:document.querySelectorAll('.weapon-card.is-flying').length,redundantLabels:[...p.querySelectorAll('h3,span')].map(x=>x.textContent.trim()).filter(x=>x==='Weapons'||/Active first|ranking data later/i.test(x)),scroll:{browser:!!browser,rail:!!rail,thumb:!!thumb,scrollbarWidth:bs.scrollbarWidth,overflowY:bs.overflowY,hasScroll:rail?.classList.contains('has-scroll')||false,progress:rail?.style.getPropertyValue('--scroll-p')||'',thumbColor:ts?.color||null},equipStyle:{background:es.backgroundImage,bgColor:es.backgroundColor,borderColor:es.borderTopColor,color:es.color},panel:{left:r.left,top:r.top,right:r.right,bottom:r.bottom,width:r.width,height:r.height},align:{panelTitle:getComputedStyle(document.getElementById('weaponTitle')).textAlign,cardCopy:getComputedStyle(document.querySelector('#weaponChoices .weapon-card-copy')).textAlign,previewCopy:getComputedStyle(document.querySelector('.weapon-preview-copy')).textAlign,previewStats:getComputedStyle(document.querySelector('.weapon-preview-stats')).textAlign,previewEffect:getComputedStyle(document.querySelector('.weapon-preview-effect')).textAlign},visual:{scrimBackground:ss.backgroundColor,panelBackground:ps.backgroundImage,panelBackdrop:ps.backdropFilter}}})()`);
  ...
  if(width>760&&(opened.align.panelTitle!=='center'||opened.align.cardCopy!=='center'||opened.align.previewCopy!=='center'||opened.align.previewStats!=='left'||opened.align.previewEffect!=='left')) throw new Error(`Weapon text hierarchy alignment failed: ${JSON.stringify(opened.align)}`);
  if(opened.redundantLabels.length) throw new Error(`Redundant Weapon browser labels returned: ${JSON.stringify(opened.redundantLabels)}`);
  ...
  if(opened.equipStyle.bgColor==='rgb(238, 238, 238)'||opened.equipStyle.color==='rgb(8, 9, 11)') throw new Error(`Disabled Equip Weapon must not regress to the old white system-button styling: ${JSON.stringify(opened.equipStyle)}`);
  ...
  const previewOnly=await evaluate(send,`(()=>{const choices=[...document.querySelectorAll('#weaponChoices .weapon-choice')],equip=document.getElementById('weaponEquip'),preview=document.querySelector('#weaponPreviewStage .weapon-preview-layer'),hero=preview?.querySelector('.weapon-preview-hero'),stage=document.getElementById('weaponPreviewStage'),hr=hero?.getBoundingClientRect(),sr=stage.getBoundingClientRect();return{currentId:weaponUi.currentId,previewId:weaponUi.previewId,order:choices.map(x=>x.dataset.weaponId),equipped:choices.filter(x=>x.classList.contains('is-equipped')).length,slotCount:document.querySelectorAll('#weaponSlotHost .weapon-card').length,previewIdDom:preview?.dataset.weaponId||null,heroId:hero?.dataset.weaponId||null,heroSvg:!!hero?.querySelector('svg'),nestedPreviewCards:stage.querySelectorAll('.weapon-card').length,heroRect:hr?{width:hr.width,height:hr.height}:null,stageRect:{width:sr.width,height:sr.height},equipText:equip.textContent.trim(),equipDisabled:equip.disabled,equipStyle:{background:getComputedStyle(equip).backgroundImage,bgColor:getComputedStyle(equip).backgroundColor,borderColor:getComputedStyle(equip).borderTopColor,color:getComputedStyle(equip).color},flying:document.querySelectorAll('.weapon-card.is-flying').length,gridCards:document.querySelectorAll('#weaponChoices .weapon-card').length}})()`);
  const baseOrder=['preview-01','preview-02','preview-03','preview-04','preview-05','preview-06','preview-07'];
  if(previewOnly.currentId!==null||previewOnly.previewId!=='preview-01'||previewOnly.equipped!==0||previewOnly.slotCount!==0||previewOnly.previewIdDom!=='preview-01'||previewOnly.heroId!=='preview-01'||!previewOnly.heroSvg||previewOnly.nestedPreviewCards!==0||!previewOnly.heroRect||previewOnly.heroRect.width<200||previewOnly.heroRect.height<200||previewOnly.equipText!=='Equip Weapon'||previewOnly.equipDisabled||!String(previewOnly.equipStyle.background).includes('linear-gradient')||previewOnly.equipStyle.bgColor==='rgb(238, 238, 238)'||previewOnly.equipStyle.color==='rgb(8, 9, 11)'||previewOnly.flying!==0||previewOnly.gridCards!==7||JSON.stringify(previewOnly.order)!==JSON.stringify(baseOrder)) throw new Error(`Weapon click did not create frameless hero Preview + dark Equip primary without mutating Active: ${JSON.stringify(previewOnly)}`);
  ...
  if(!specOk||!traceOk||tunnel.currentId!=='preview-01'||tunnel.previewId!=='preview-02'||tunnel.firstId!=='preview-01'||tunnel.buildCards!==1||tunnel.buildCardId!=='preview-01'||tunnel.buttonLabel!=='Open Weapon menu. Active: Weapon 01'||tunnel.flying!==0||tunnel.gridCards!==7||tunnel.nestedPreviewCards!==0) throw new Error(`Weapon hero Preview tunnel changed Active/grid/Build state or reintroduced nested cards: ${JSON.stringify(tunnel)}`);
  if(!tunnel.reduced&&(tunnel.count!==2||tunnel.incomingId!=='preview-02'||tunnel.outgoingId!=='preview-06'||tunnel.incomingHero!=='preview-02'||tunnel.outgoingHero!=='preview-06'||!String(tunnel.incomingFrom).includes('scale(0.64)')||!String(tunnel.incomingTo).includes('scale(1)')||!String(tunnel.outgoingTo).includes('scale(1.24)')||String(tunnel.outgoingOpacity)!=='0')) throw new Error(`Weapon hero Preview live tunnel layers failed: ${JSON.stringify(tunnel)}`);
...
try{
  ...
  try{
    ...
    console.log(`- Weapon selector cleanup passed: redundant browser labels removed, native scrollbar hidden behind the custom black rail/gold star indicator, dark Equip primary applied, and locked Preview/Equip/tunnel/no-flight behavior preserved; desktop equipped: ${desktopWeapon.equipped}, mobile smoke equipped: ${mobileWeapon.equipped}.`);
