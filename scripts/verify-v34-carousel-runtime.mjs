...
async function verifyWeaponOverlay(send, width, height, capturePath) {
  ...
  await sleep(540);
  const secondPreview=await evaluate(send,`(()=>{const p=document.getElementById('weaponPanel'),preview=document.querySelector('#weaponPreviewHost .weapon-card'),current=document.querySelector('#weaponCurrentHost .weapon-card'),source=document.querySelector('#weaponChoices .weapon-choice[data-weapon-id="preview-02"]');return{hasPreview:p.classList.contains('has-preview'),previewId:preview?.dataset.weaponId||null,currentId:current?.dataset.weaponId||null,currentName:document.getElementById('weaponCurrentName').textContent.trim(),sourceGhost:source?.classList.contains('is-preview-source')||false}})()`);
