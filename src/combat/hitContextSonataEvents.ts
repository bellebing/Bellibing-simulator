...
export interface ProvenHitSonataDamage {
  readonly effectId: 'S22_3PC_HEAVY_CR' | 'S29_5PC_AERO';
...
export function evaluateHitContextSonataCasts(input: {
  ...
  readonly hitDamageClass: DirectHitDamageClass;
  ...
  const damages = (proof.damages ?? []).map(c => {
    ...
    if (rows.length !== 1 || !['S22_3PC_HEAVY_CR', 'S29_5PC_AERO'].includes(c.effectId)
      || pieces < effect.pieces || !text(c.evidenceId)
      || c.equipmentAtEventQualified !== true || c.priorActivationState !== 'NONE_ACTIVE'
      || c.noLaterActivationThroughHit !== true || !['BEFORE_TRIGGER', 'AFTER_TRIGGER'].includes(c.sameTimestampOrder)
      || !c.event || c.event.actorId !== input.characterId || input.hitAtSeconds < c.event.atSeconds) {
      throw new Error('Require exact equipped Sonata pieces/owner, proven damage and isolated query ordering');
    }
    const window = activateSonataDamageWindow({ effectId: c.effectId, ownerId: input.characterId,
      selectedSet: { id: effect.sonataSetId, pieces }, event: c.event });
    if (!window) throw new Error('The supplied damage event does not activate this canonical Sonata effect');
    const active = isSonataDamageWindowActive(window, { actorId: input.characterId, atSeconds: input.hitAtSeconds,
      sameTimestampOrder: c.sameTimestampOrder });
    const heavyCrit = c.effectId === 'S22_3PC_HEAVY_CR';
    if ((heavyCrit && window.statOrEffect !== 'Heavy Attack CRIT Rate')
      || (!heavyCrit && window.statOrEffect !== 'Aero DMG Bonus')) {
      throw new Error('Reviewed Sonata damage-hit stat contract drift');
    }
    const appliesToSelectedHit = !heavyCrit || input.hitDamageClass === 'HEAVY';
    return { sourceId: `sonata:${c.effectId}`, stat: heavyCrit ? 'CRIT Rate' : window.statOrEffect,
      value: active && appliesToSelectedHit ? window.value : 0,
      status: 'EVENT_QUALIFIED_ASSEMBLED' as const, active, appliesToSelectedHit, evidenceId: c.evidenceId,
      magnitudeDependsOnEchoStats: false as const, activationProof: 'PER_BUILD_EXPLICIT_EVENT' as const,
      sourceKey: JSON.stringify(effect), window: { ...window } };
  });
