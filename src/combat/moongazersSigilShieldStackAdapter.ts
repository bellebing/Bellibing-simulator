...
export const MOONGAZERS_SIGIL_SHIELD_STACK_CONTRACT = {
...
export interface MoongazersSigilKnownShieldState {
  ...
  /** One expiry timestamp for each source-known organic MGS-DEF stack. */
  readonly organicStackExpiriesSeconds: readonly number[];
  /** Latest accepted owner Shield trigger, even if its stack later expires. */
...
export type MoongazersSigilShieldEventResult =
  | { readonly status: 'IGNORED_OTHER_ACTOR'; readonly state: MoongazersSigilKnownShieldState }
  | { readonly status: 'TRIGGER_COOLDOWN'; readonly state: MoongazersSigilKnownShieldState }
  | {
...
export const MOONGAZERS_SIGIL_SHIELD_STACK_SEMANTIC_REVIEW = {
  ...
  notes: [
    'The organic primitive executes only explicit owner Shield events from caller-supplied monotonic known state and fails closed on a valid at-cap event.',
    'The Intro primitive is a separate three-second forced-max read window; it never fabricates five seven-second organic stack expiries.',
...
export function validateMoongazersSigilShieldStackContract(
  ...
  const issues: string[] = [];
  const c = MOONGAZERS_SIGIL_SHIELD_STACK_CONTRACT;
  const shield = uniqueEffect(catalog, c.shieldEffectId);
  const intro = uniqueEffect(catalog, c.introEffectId);

  if (!shield) {
    issues.push(`missing weapon effect ${c.shieldEffectId}`);
  } else {
    if (shield.weaponId !== c.weaponId) issues.push('MGS-DEF weapon identity drift');
    ...
    if (shield.durationSeconds !== c.shieldDurationSeconds) issues.push(`MGS-DEF duration drift: ${String(shield.durationSeconds)}`);
    if (shield.triggerCooldownSeconds !== c.triggerCooldownSeconds) issues.push(`MGS-DEF trigger cooldown drift: ${String(shield.triggerCooldownSeconds)}`);
    if (shield.stackIntervalSeconds !== c.triggerCooldownSeconds) issues.push(`MGS-DEF stack interval drift: ${String(shield.stackIntervalSeconds)}`);
    if (shield.maxStacks !== c.maxStacks) issues.push(`MGS-DEF maxStacks drift: ${shield.maxStacks}`);
    ...
    if (shield.rankValues.length !== c.rankValuesPerStack.length || shield.rankValues.some((value, index) => value !== c.rankValuesPerStack[index])) {
      issues.push('MGS-DEF rank values drift');
    }
  }

  if (!intro) {
    issues.push(`missing weapon effect ${c.introEffectId}`);
  } else {
    if (intro.weaponId !== c.weaponId) issues.push('MGS-MAX-STACK weapon identity drift');
    ...
    if (intro.durationSeconds !== c.introForcedMaxDurationSeconds) issues.push(`MGS-MAX-STACK duration drift: ${String(intro.durationSeconds)}`);
    ...
    if (!intro.rankValues.every((value) => value === c.maxStacks)) issues.push('MGS-MAX-STACK must force five stacks at every rank');
  }

  const unresolved: readonly string[] = MOONGAZERS_SIGIL_SHIELD_STACK_SEMANTIC_REVIEW.unresolvedSemantics;
  if (unresolved.length === 0) issues.push('Moongazer shield-stack review must retain explicit unresolved semantics');
  return issues;
}

const CONTRACT_ISSUES = validateMoongazersSigilShieldStackContract();
if (CONTRACT_ISSUES.length > 0) throw new Error(`Invalid Moongazer shield-stack contract: ${CONTRACT_ISSUES.join('; ')}`);
...
function activeExpiries(state: MoongazersSigilKnownShieldState, atSeconds: number): readonly number[] {
  return state.organicStackExpiriesSeconds.filter((expiry) => expiry > atSeconds);
}
...
export function createMoongazersSigilKnownShieldState(params: {
  ...
  const { ownerId, knownAtSeconds, organicStackExpiriesSeconds, lastAcceptedShieldAtSeconds } = params;
  const c = MOONGAZERS_SIGIL_SHIELD_STACK_CONTRACT;
  ...
  if (organicStackExpiriesSeconds.length > c.maxStacks) throw new Error(`Moongazer known organic stack count cannot exceed ${c.maxStacks}`);

  const expiries = [...organicStackExpiriesSeconds].sort((a, b) => a - b);
  ...
  if (expiries.length > 0 && lastAcceptedShieldAtSeconds === null) {
    throw new Error('Moongazer active organic stacks require a known last accepted Shield event for cooldown-safe continuation');
  }
  if (expiries.length > 0 && lastAcceptedShieldAtSeconds !== null) {
    const expectedLatestExpiry = lastAcceptedShieldAtSeconds + c.shieldDurationSeconds;
    const latestExpiry = expiries[expiries.length - 1];
    if (Math.abs(latestExpiry - expectedLatestExpiry) > 1e-12) {
      throw new Error('Moongazer active organic stacks must include the seven-second expiry of the known latest accepted Shield event');
    }
  }
  ...
  return {
    ...
    ownerId,
    knownAtSeconds,
    organicStackExpiriesSeconds: expiries,
...
export function applyMoongazersSigilShieldEvent(
  ...
  if (event.atSeconds < state.knownAtSeconds) throw new Error('Moongazer Shield events must not move backward before known state time');

  const pruned: MoongazersSigilKnownShieldState = {
    ...state,
    knownAtSeconds: event.atSeconds,
    organicStackExpiriesSeconds: activeExpiries(state, event.atSeconds),
  };
  if (event.actorId !== state.ownerId) return { status: 'IGNORED_OTHER_ACTOR', state: pruned };

  const last = state.lastAcceptedShieldAtSeconds;
  if (last !== null && event.atSeconds - last < c.triggerCooldownSeconds) return { status: 'TRIGGER_COOLDOWN', state: pruned };
  if (pruned.organicStackExpiriesSeconds.length >= c.maxStacks) {
    return {
  ...
  const expiry = event.atSeconds + c.shieldDurationSeconds;
  return {
    status: 'STACK_GRANTED',
    grantedStackExpiresAtSeconds: expiry,
    state: {
      ...pruned,
      organicStackExpiriesSeconds: [...pruned.organicStackExpiriesSeconds, expiry].sort((a, b) => a - b),
...
export function resolveMoongazersSigilOrganicDefIgnore(params: {
  ...
  if (atSeconds < state.knownAtSeconds) throw new Error('Moongazer DEF-ignore query cannot move backward before known state time');
  if (!Number.isInteger(rank) || rank < 1 || rank > 5) throw new Error(`Moongazer rank must be an integer from 1 through 5: ${rank}`);
  const activeOrganicStackCount = activeExpiries(state, atSeconds).length;
...
export function activateMoongazersSigilIntroForcedMaxWindow(params: {
  readonly ownerId: string;
  readonly event: { readonly actorId: string; readonly atSeconds: number };
...
export function isMoongazersSigilIntroForcedMaxActive(window: MoongazersSigilForcedMaxWindow, atSeconds: number): boolean {
