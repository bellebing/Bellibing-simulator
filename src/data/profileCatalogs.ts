import { CHARACTER_BUILD_PRESETS } from './characterBuildPresets.ts';
import { ECHO_LOADOUT_PROFILES } from './echoLoadoutProfiles.ts';
import {
  PROFILE_COHORT_01_GREEN_LANE_ECHOES,
  PROFILE_COHORT_01_GREEN_LANE_PRESETS,
  PROFILE_COHORT_01_GREEN_LANE_ROTATIONS,
  PROFILE_COHORT_01_GREEN_LANE_STATS,
  PROFILE_COHORT_01_GREEN_LANE_TEAMS,
  PROFILE_COHORT_01_GREEN_LANE_WEAPONS,
} from './profileCohort01GreenLane20260830.ts';
import {
  PROFILE_HORIZONTAL_GREEN_LANE_ECHOES,
  PROFILE_HORIZONTAL_GREEN_LANE_PRESETS,
  PROFILE_HORIZONTAL_GREEN_LANE_ROTATIONS,
  PROFILE_HORIZONTAL_GREEN_LANE_STATS,
  PROFILE_HORIZONTAL_GREEN_LANE_TEAMS,
  PROFILE_HORIZONTAL_GREEN_LANE_WEAPONS,
} from './profileHorizontalGreenLane20260830.ts';
import {
  PROFILE_HORIZONTAL_GREEN_LANE_ECHOES as PROFILE_HORIZONTAL_20260831_ECHOES,
  PROFILE_HORIZONTAL_GREEN_LANE_PRESETS as PROFILE_HORIZONTAL_20260831_PRESETS,
  PROFILE_HORIZONTAL_GREEN_LANE_ROTATIONS as PROFILE_HORIZONTAL_20260831_ROTATIONS,
  PROFILE_HORIZONTAL_GREEN_LANE_STATS as PROFILE_HORIZONTAL_20260831_STATS,
  PROFILE_HORIZONTAL_GREEN_LANE_TEAMS as PROFILE_HORIZONTAL_20260831_TEAMS,
  PROFILE_HORIZONTAL_GREEN_LANE_WEAPONS as PROFILE_HORIZONTAL_20260831_WEAPONS,
} from './profileHorizontalGreenLane20260831/index.ts';
import {
  PROFILE_MULTIMODE_AALTO_ECHOES,
  PROFILE_MULTIMODE_AALTO_PRESETS,
  PROFILE_MULTIMODE_AALTO_ROTATIONS,
  PROFILE_MULTIMODE_AALTO_STATS,
  PROFILE_MULTIMODE_AALTO_TEAMS,
  PROFILE_MULTIMODE_AALTO_WEAPONS,
} from './profileMultiModeAalto20260829.ts';
import {
  PROFILE_MULTIMODE_DENIA_ECHOES,
  PROFILE_MULTIMODE_DENIA_PRESETS,
  PROFILE_MULTIMODE_DENIA_ROTATIONS,
  PROFILE_MULTIMODE_DENIA_STATS,
  PROFILE_MULTIMODE_DENIA_WEAPONS,
} from './profileMultiModeDenia20260829.ts';
import {
  PROFILE_MULTIMODE_ZHEZHI_ECHOES,
  PROFILE_MULTIMODE_ZHEZHI_PRESETS,
  PROFILE_MULTIMODE_ZHEZHI_ROTATIONS,
  PROFILE_MULTIMODE_ZHEZHI_STATS,
  PROFILE_MULTIMODE_ZHEZHI_TEAMS,
  PROFILE_MULTIMODE_ZHEZHI_WEAPONS,
} from './profileMultiModeZhezhi20260829.ts';
import {
  PROFILE_SOURCE_BATCH_20260829_ECHOES,
  PROFILE_SOURCE_BATCH_20260829_PRESETS,
  PROFILE_SOURCE_BATCH_20260829_ROTATIONS,
  PROFILE_SOURCE_BATCH_20260829_STATS,
  PROFILE_SOURCE_BATCH_20260829_TEAMS,
  PROFILE_SOURCE_BATCH_20260829_WEAPONS,
} from './profileSourceBatch20260829.ts';
import { ROTATION_PROFILES } from './rotationProfiles.ts';
import { STAT_TARGET_PROFILES } from './statTargetProfiles.ts';
import { TEAM_PROFILES } from './teamProfiles.ts';
import { WEAPON_RECOMMENDATION_PROFILES } from './weaponRecommendations.ts';
import type { ProfileCatalogs } from '../profileDomain.ts';
import { createProfileRegistry } from '../profileRegistry.ts';

export const PROFILE_CATALOGS: ProfileCatalogs = {
  weaponRecommendations: [
    ...WEAPON_RECOMMENDATION_PROFILES,
    ...PROFILE_SOURCE_BATCH_20260829_WEAPONS,
    ...PROFILE_MULTIMODE_AALTO_WEAPONS,
    ...PROFILE_MULTIMODE_ZHEZHI_WEAPONS,
    ...PROFILE_MULTIMODE_DENIA_WEAPONS,
    ...PROFILE_COHORT_01_GREEN_LANE_WEAPONS,
    ...PROFILE_HORIZONTAL_GREEN_LANE_WEAPONS,
    ...PROFILE_HORIZONTAL_20260831_WEAPONS,
  ],
  echoLoadouts: [
    ...ECHO_LOADOUT_PROFILES,
    ...PROFILE_SOURCE_BATCH_20260829_ECHOES,
    ...PROFILE_MULTIMODE_AALTO_ECHOES,
    ...PROFILE_MULTIMODE_ZHEZHI_ECHOES,
    ...PROFILE_MULTIMODE_DENIA_ECHOES,
    ...PROFILE_COHORT_01_GREEN_LANE_ECHOES,
    ...PROFILE_HORIZONTAL_GREEN_LANE_ECHOES,
    ...PROFILE_HORIZONTAL_20260831_ECHOES,
  ],
  statTargets: [
    ...STAT_TARGET_PROFILES,
    ...PROFILE_SOURCE_BATCH_20260829_STATS,
    ...PROFILE_MULTIMODE_AALTO_STATS,
    ...PROFILE_MULTIMODE_ZHEZHI_STATS,
    ...PROFILE_MULTIMODE_DENIA_STATS,
    ...PROFILE_COHORT_01_GREEN_LANE_STATS,
    ...PROFILE_HORIZONTAL_GREEN_LANE_STATS,
    ...PROFILE_HORIZONTAL_20260831_STATS,
  ],
  teams: [
    ...TEAM_PROFILES,
    ...PROFILE_SOURCE_BATCH_20260829_TEAMS,
    ...PROFILE_MULTIMODE_AALTO_TEAMS,
    ...PROFILE_MULTIMODE_ZHEZHI_TEAMS,
    ...PROFILE_COHORT_01_GREEN_LANE_TEAMS,
    ...PROFILE_HORIZONTAL_GREEN_LANE_TEAMS,
    ...PROFILE_HORIZONTAL_20260831_TEAMS,
  ],
  rotations: [
    ...ROTATION_PROFILES,
    ...PROFILE_SOURCE_BATCH_20260829_ROTATIONS,
    ...PROFILE_MULTIMODE_AALTO_ROTATIONS,
    ...PROFILE_MULTIMODE_ZHEZHI_ROTATIONS,
    ...PROFILE_MULTIMODE_DENIA_ROTATIONS,
    ...PROFILE_COHORT_01_GREEN_LANE_ROTATIONS,
    ...PROFILE_HORIZONTAL_GREEN_LANE_ROTATIONS,
    ...PROFILE_HORIZONTAL_20260831_ROTATIONS,
  ],
  presets: [
    ...CHARACTER_BUILD_PRESETS,
    ...PROFILE_SOURCE_BATCH_20260829_PRESETS,
    ...PROFILE_MULTIMODE_AALTO_PRESETS,
    ...PROFILE_MULTIMODE_ZHEZHI_PRESETS,
    ...PROFILE_MULTIMODE_DENIA_PRESETS,
    ...PROFILE_COHORT_01_GREEN_LANE_PRESETS,
    ...PROFILE_HORIZONTAL_GREEN_LANE_PRESETS,
    ...PROFILE_HORIZONTAL_20260831_PRESETS,
  ],
};

/** Canonical resolved profile registry consumed by future UI/engines. */
export const PROFILE_REGISTRY = createProfileRegistry(PROFILE_CATALOGS);
