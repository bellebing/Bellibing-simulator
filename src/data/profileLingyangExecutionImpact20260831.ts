...
export const PROFILE_LINGYANG_EXECUTION_IMPACT_REVIEWS: readonly ProfileBackwardImpactReview[] = [
  {
    ...
    notes: [
      ...
      'Moongazer’s Sigil permanent 12% ATK is executable source truth and MGS-LIB already has the reusable explicit cast-window primitive. Profile-specific trigger identity is now also exact: the canonical Lingyang sequence contains Intro at step index 1 and Ultimate / Strive: Lion’s Vigor at step index 2, matching the source MGS-LIB trigger Cast Intro Skill or Resonance Liberation. `LINGYANG-MOONGAZERS-SIGIL-CAST-TRIGGER-REVIEW-2026-09-01-01` records those two checkpoints only; it does not instantiate windows because SOURCE_SEQUENCE_ONLY supplies neither timestamp, and two qualifying casts also require a source-safe same-effect retrigger / refresh / overlap lifecycle before canonical uptime can execute. MGS-DEF has a separate source-safe partial primitive: explicit owner Shield events from caller-supplied monotonic known state grant independent seven-second stacks at most once every 0.5s until five active stacks; a valid event at cap fails closed because refresh/replacement/ignore semantics are not source-established. Intro creates only a separate three-second forced-max read window and never fabricates five organic seven-second expiries. All three Moongazer pending IDs remain open.',
      ...
      'Mech Abomination Rank-5 attack math is exact: 48.64% ATK front strike plus 320% and 160% ATK Mech Waste components; Waste damage is explicitly Outro Skill DMG. A source-safe explicit-cast primitive turns only a caller-supplied Mech cast timestamp into the source-backed +12% ATK [cast, cast+15s) wielder window and next-cast readiness at cast+20s. `LINGYANG-MECH-ABOMINATION-CAST-CHECKPOINT-REVIEW-2026-09-01-01` now profile-binds canonical source step index 0 `Echo: Mech Abomination` to the exact `ECHO_ACTIVE_CAST` identity for echo-60000485, but deliberately does not interpret first sequence position as t=0. The exact attack IDs remain unscheduled because current source does not provide cast-to-front-strike/Waste-hit/explosion delays. The canonical Mech timeline pending ID therefore remains open on numeric cast timing plus hit timeline.',
      ...
      '`LINGYANG-ER-GATE-REVIEW-2026-09-01-01` verifies that canonical lingyang-standard-stats keeps Energy Regen as priority 1 but materializes `gates: []`. The reviewed 120–125%+ Energy Regen text is estimated Zhezhi + The Shorekeeper team context only, and canonical provenance explicitly says no exact numeric ER gate is claimed/materialized. No 120%, 125% or other boundary value is promoted; the exact-ER pending ID remains BLOCKED_SOURCE_SEMANTICS.',
