...
export const PROFILE_LINGYANG_EXECUTION_IMPACT_REVIEWS: readonly ProfileBackwardImpactReview[] = [
  {
    ...
    notes: [
      ...
      'Moongazer’s Sigil permanent 12% ATK is executable source truth. Liberation DMG uptime, shield-driven DEF-ignore stacks, and the Intro-triggered three-second max-stack override require an exact timeline/external shield state; source sequence adjacency is not treated as elapsed-time proof.',
      'Lingering Tunes identity is already mapped from the reviewed source label Endless Resonance to canonical sonata-9. Runtime truth is narrower: 5% ATK per 1.5s while on field, cap 4, plus 60% Outro Skill DMG. The current source does not prove a post-field stack lifetime/refresh rule, and no fixed full-stack uptime is authorized.',
