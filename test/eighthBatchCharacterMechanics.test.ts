...
test('eighth Character Mechanics batch advances canonical coverage to 28 verified / 29 unstarted / 889 facts', () => {
  const audit = auditCharacterMechanicsCoverage();
  assert.equal(audit.releasedCount, 57);
  assert.equal(audit.profileCount, 28);
  assert.equal(audit.verifiedCharacterIds.length, 28);
