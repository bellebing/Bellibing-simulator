...
test('Molten Rift cast-window contract stays locked to the exact canonical Sonata row', () => {
  ...
  const drifted = SONATA_EFFECT_MODELS.map((effect) => effect.effectId === 'S02_5PC_FUSION'
    ? { ...effect, durationSeconds: 14 }
    : effect);
  assert.ok(validateSonataCastWindowContracts(drifted).some((issue) => issue.includes('duration drift')));
});
...
test('Molten Rift active-window boundary is deterministic and fail closed', () => {
  const window = activateSonataCastWindow({
    effectId: 'S02_5PC_FUSION',
    ownerId: 'changli',
