...
test('adding audited effect data requires removing the weapon from the pending-source set', () => {
  ...
  assert.ok(source);
  const syntheticEffect = { ...source, effectId: 'TEST-PENDING-OVERLAP', weaponId: 'glint-of-clouds' };
...
function currentProfileIds(weaponType: string): string[] {
  return WEAPON_RECOMMENDATION_PROFILES
    .filter((profile) => CHARACTER_CATALOG.find((row) => row.id === profile.characterId)?.weaponType === weaponType)
...
test('Rectifier effect batch 1 screens the exact released Rectifier roster without creating profiles', () => {
  ...
  assert.equal(review.result, 'REVIEWED_NO_EXISTING_PROFILE_CHANGE');
  for (const weaponId of review.weaponIds) assert.equal(getWeaponEffectCoverageStatus(weaponId), 'AUDITED_EFFECTS', weaponId);
});
...
test('Rectifier effect batch 2 repeats the exact released Rectifier backward-impact screen', () => {
  ...
  assert.deepEqual(releasedIds, [...EXPECTED_RELEASED_RECTIFIER_CHARACTERS]);
  assert.deepEqual([...review.reviewedReleasedCharacterIds].sort(), releasedIds);
  assert.deepEqual([...review.existingWeaponRecommendationProfileIds].sort(), currentProfileIds('Rectifier'));
...
test('Broadblade completion closes all 23 released weapons and preserves Augusta profile relations', () => {
  ...
  assert.equal(review.result, 'REVIEWED_NO_EXISTING_PROFILE_CHANGE');
  assert.deepEqual(review.weaponIds, [
    'radiance-cleaver', 'thunderflare-dominion', 'aureate-zenith', 'broadblade-41', 'dauntless-evernight',
    'discord', 'meditations-on-mercy', 'waning-redshift', 'beguiling-melody', 'broadblade-of-night',
    'broadblade-of-voyager', 'guardian-broadblade', 'originite-type-i', 'tyro-broadblade', 'training-broadblade',
  ]);
  ...
  assert.deepEqual(augustaProfile.options.map((option) => [option.weaponId, option.rank, option.relativePerformance]), [
    ['thunderflare-dominion', 1, 1], ['verdant-summit', 1, .903], ['ages-of-harvest', 1, .804],
    ['wildfire-mark', 1, .773], ['radiance-cleaver', 1, .773], ['kumokiri', 1, .772],
    ['lustrous-razor', 1, .743], ['aureate-zenith', 5, .732], ['autumntrace', 5, .71],
    ['waning-redshift', 5, .665], ['helios-cleaver', 5, .639], ['meditations-on-mercy', 5, .607],
  ]);
...
test('Gauntlet completion closes all 22 released weapons and screens the exact 10-character roster', () => {
  ...
  assert.equal(review.result, 'REVIEWED_NO_EXISTING_PROFILE_CHANGE');
  assert.deepEqual(review.weaponIds, [
    'abyss-surges', 'blazing-justice', 'daybreakers-spine', 'moongazers-sigil', 'pulsation-bracer',
    'solsworn-ciphers', 'tragicomedy', 'veritys-handle', 'aether-strike', 'amity-accord', 'celestial-spiral',
    'gauntlets-21d', 'hollow-mirage', 'legend-of-drunken-hero', 'marcato', 'stonard', 'gauntlets-of-night',
    'gauntlets-of-voyager', 'guardian-gauntlets', 'originite-type-iv', 'tyro-gauntlets', 'training-gauntlets',
  ]);
