...
test('future released weapon fails the effect coverage gate until its status is explicitly audited', () => {
  ...
  const synthetic = { ...glint, id: 'test-only-future-effect-weapon', name: 'TEST ONLY — future effect weapon' };
...
function releasedCharacterIds(weaponType: string): string[] {
  return CHARACTER_CATALOG.filter((character) => character.releaseStatus === 'RELEASED' && character.weaponType === weaponType).map((character) => character.id).sort();
}
function currentProfileIds(weaponType: string): string[] {
  return WEAPON_RECOMMENDATION_PROFILES.filter((profile) => CHARACTER_CATALOG.find((row) => row.id === profile.characterId)?.weaponType === weaponType).map((profile) => profile.id).sort();
}
function releasedWeaponIds(weaponType: string): string[] {
  return WEAPON_CATALOG.filter((weapon) => weapon.releaseStatus === 'RELEASED' && weapon.weaponType === weaponType).map((weapon) => weapon.id).sort();
}

const EXPECTED_RELEASED_PISTOL_CHARACTERS = ['aalto', 'carlotta', 'chixia', 'ciaccona', 'galbrena', 'lucy', 'lynae', 'mortefi', 'rebecca'] as const;
const EXPECTED_RELEASED_RECTIFIER_CHARACTERS = ['baizhi', 'buling', 'cantarella', 'denia', 'encore', 'lucilla', 'phoebe', 'phrolova', 'suisui', 'the-shorekeeper', 'verina', 'yinlin', 'zhezhi'] as const;
const EXPECTED_RELEASED_BROADBLADE_CHARACTERS = ['augusta', 'calcharo', 'chisa', 'jinhsi', 'jiyan', 'lumi', 'lupa', 'mornye', 'taoqi'] as const;
const EXPECTED_RELEASED_GAUNTLET_CHARACTERS = ['iuno', 'jianxin', 'lingyang', 'luuk-herssen', 'roccia', 'sigrika', 'xiangli-yao', 'youhu', 'yuanwu', 'zani'] as const;
const EXPECTED_RELEASED_SWORD_CHARACTERS = ['aemeath', 'brant', 'camellya', 'cartethyia', 'changli', 'danjin', 'hiyuki', 'qingxiao', 'qiuyuan', 'rover-aero', 'rover-electro', 'rover-havoc', 'rover-spectro', 'sanhua', 'yangyang', 'yangyang-xuanling'] as const;

test('Pistol effect batch 1 has an exact roster-wide backward-impact review', () => {
  const review = WEAPON_EFFECT_BACKWARD_IMPACT_REVIEWS_V36.find((row) => row.reviewId === 'WEAPON-EFFECT-PISTOLS-2026-08-25-01');
  assert.ok(review);
  ...
  assert.deepEqual(currentProfileIds('Pistols'), []);
...
test('Pistol effect batch 2 repeats the full backward-impact screen', () => {
  const review = WEAPON_EFFECT_BACKWARD_IMPACT_REVIEWS_V36.find((row) => row.reviewId === 'WEAPON-EFFECT-PISTOLS-2026-08-25-02');
  assert.ok(review);
  ...
  assert.equal(review.result, 'REVIEWED_NO_EXISTING_PROFILE_CHANGE');
  assert.deepEqual(review.weaponIds, ['cadenza', 'pistols-of-voyager', 'pistols-of-night', 'guardian-pistols', 'originite-type-iii', 'tyro-pistols', 'training-pistols', 'undying-flame', 'novaburst', 'thunderbolt']);
});

test('Pistol effect batch 3 closes released Pistol coverage', () => {
  const review = WEAPON_EFFECT_BACKWARD_IMPACT_REVIEWS_V36.find((row) => row.reviewId === 'WEAPON-EFFECT-PISTOLS-2026-08-25-03');
  assert.ok(review);
  assert.deepEqual([...review.reviewedReleasedCharacterIds].sort(), releasedCharacterIds('Pistols'));
  assert.deepEqual([...review.existingWeaponRecommendationProfileIds].sort(), currentProfileIds('Pistols'));
  assert.deepEqual(review.weaponIds, ['lux-and-umbra', 'phasic-homogenizer', 'skull-thrasher', 'spectral-trigger', 'static-mist', 'the-last-dance', 'pistols-26', 'romance-in-farewell', 'solar-flame']);
  assert.equal(review.result, 'REVIEWED_NO_EXISTING_PROFILE_CHANGE');
  const releasedWeapons = releasedWeaponIds('Pistols');
  assert.equal(releasedWeapons.length, 22);
  for (const weaponId of releasedWeapons) assert.equal(getWeaponEffectCoverageStatus(weaponId), 'AUDITED_EFFECTS', weaponId);
});

test('Rectifier effect batch 1 screens the exact released Rectifier roster without creating profiles', () => {
  const review = WEAPON_EFFECT_BACKWARD_IMPACT_REVIEWS_V36.find((row) => row.reviewId === 'WEAPON-EFFECT-RECTIFIERS-2026-08-25-01');
  assert.ok(review);
  ...
  assert.deepEqual([...review.existingWeaponRecommendationProfileIds].sort(), []);
  assert.deepEqual(review.weaponIds, ['boson-astrolabe', 'cosmic-ripples', 'firstlights-herald', 'luminous-hymn', 'stellar-symphony']);
  assert.equal(review.result, 'REVIEWED_NO_EXISTING_PROFILE_CHANGE');
});

test('Rectifier effect batch 2 repeats the exact released Rectifier backward-impact screen', () => {
  const review = WEAPON_EFFECT_BACKWARD_IMPACT_REVIEWS_V36.find((row) => row.reviewId === 'WEAPON-EFFECT-RECTIFIERS-2026-08-25-02');
  assert.ok(review);
  assert.deepEqual([...review.reviewedReleasedCharacterIds].sort(), releasedCharacterIds('Rectifier'));
  assert.deepEqual(currentProfileIds('Rectifier'), []);
  assert.deepEqual(review.weaponIds, ['augment', 'call-of-the-abyss', 'comet-flare', 'fusion-accretion', 'jinzhou-keeper']);
  assert.equal(review.result, 'REVIEWED_NO_EXISTING_PROFILE_CHANGE');
});

test('Rectifier batches 3 and 4 close released Rectifier coverage', () => {
  const batch3 = WEAPON_EFFECT_BACKWARD_IMPACT_REVIEWS_V36.find((row) => row.reviewId === 'WEAPON-EFFECT-RECTIFIERS-2026-08-25-03');
  const batch4 = WEAPON_EFFECT_BACKWARD_IMPACT_REVIEWS_V36.find((row) => row.reviewId === 'WEAPON-EFFECT-RECTIFIERS-2026-08-25-04');
  ...
  assert.deepEqual(batch3.weaponIds, ['oceans-gift', 'radiant-dawn', 'rectifier-25', 'variation', 'waltz-in-masquerade']);
  assert.deepEqual(batch4.weaponIds, ['guardian-rectifier', 'originite-type-v', 'rectifier-of-night', 'rectifier-of-voyager', 'tyro-rectifier', 'training-rectifier']);
...
test('Broadblade completion closes all 23 released weapons and preserves Augusta profile relations', () => {
  const review = WEAPON_EFFECT_BACKWARD_IMPACT_REVIEWS_V36.find((row) => row.reviewId === 'WEAPON-EFFECT-BROADBLADES-2026-08-25-01');
  assert.ok(review);
  ...
  assert.equal(review.result, 'REVIEWED_NO_EXISTING_PROFILE_CHANGE');
  assert.deepEqual(review.weaponIds, ['radiance-cleaver', 'thunderflare-dominion', 'aureate-zenith', 'broadblade-41', 'dauntless-evernight', 'discord', 'meditations-on-mercy', 'waning-redshift', 'beguiling-melody', 'broadblade-of-night', 'broadblade-of-voyager', 'guardian-broadblade', 'originite-type-i', 'tyro-broadblade', 'training-broadblade']);
  ...
  assert.deepEqual(augustaProfile.options.map((option) => [option.weaponId, option.rank, option.relativePerformance]), [
    ['thunderflare-dominion', 1, 1], ['verdant-summit', 1, .903], ['ages-of-harvest', 1, .804], ['wildfire-mark', 1, .773], ['radiance-cleaver', 1, .773], ['kumokiri', 1, .772], ['lustrous-razor', 1, .743], ['aureate-zenith', 5, .732], ['autumntrace', 5, .71], ['waning-redshift', 5, .665], ['helios-cleaver', 5, .639], ['meditations-on-mercy', 5, .607],
  ]);
...
test('Gauntlet completion closes all 22 released weapons and screens the exact 10-character roster', () => {
  const review = WEAPON_EFFECT_BACKWARD_IMPACT_REVIEWS_V36.find((row) => row.reviewId === 'WEAPON-EFFECT-GAUNTLETS-2026-08-25-01');
  assert.ok(review);
  ...
  assert.deepEqual([...review.existingWeaponRecommendationProfileIds].sort(), []);
  assert.equal(review.result, 'REVIEWED_NO_EXISTING_PROFILE_CHANGE');
  assert.deepEqual(review.weaponIds, ['abyss-surges', 'blazing-justice', 'daybreakers-spine', 'moongazers-sigil', 'pulsation-bracer', 'solsworn-ciphers', 'tragicomedy', 'veritys-handle', 'aether-strike', 'amity-accord', 'celestial-spiral', 'gauntlets-21d', 'hollow-mirage', 'legend-of-drunken-hero', 'marcato', 'stonard', 'gauntlets-of-night', 'gauntlets-of-voyager', 'guardian-gauntlets', 'originite-type-iv', 'tyro-gauntlets', 'training-gauntlets']);
...
test('Sword completion closes all 27 released weapons and screens the exact 16-character roster', () => {
  const review = WEAPON_EFFECT_BACKWARD_IMPACT_REVIEWS_V36.find((row) => row.reviewId === 'WEAPON-EFFECT-SWORDS-2026-08-26-01');
  assert.ok(review);
  ...
  for (const weaponId of releasedWeapons) {
    ...
    assert.equal((WEAPON_EFFECT_PENDING_SOURCE_AUDIT_IDS_V36 as readonly string[]).includes(weaponId), false, weaponId);
  }
});

test('coverage lookup distinguishes audited, upcoming and unknown weapons after source completion', () => {
  for (const weaponId of ['stringmaster', 'static-mist', 'solar-flame', 'boson-astrolabe', 'training-rectifier', 'thunderflare-dominion', 'training-broadblade', 'abyss-surges', 'training-gauntlets', 'glint-of-clouds', 'training-sword']) {
    assert.equal(getWeaponEffectCoverageStatus(weaponId), 'AUDITED_EFFECTS', weaponId);
  }
